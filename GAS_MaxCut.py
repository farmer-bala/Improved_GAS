...
def draw_p(y1,y2,p):
    #插值必须是长度为2的二元组
    plt.title("Mix-alogrithm,p={}".format(p))
    plt.xlabel("times")
    plt.ylabel("total_r")
...
def draw_v(y1,y2,y3):
    #插值必须是长度为2的二元组
    plt.title("Mix-alogrithm-maxcut,p=2")
    plt.xlabel("times")
    plt.ylabel("fx")
    ...
    plt.legend()
    plt.show()
def matrix_generator(graph,lAbmda,y,B,p,shots = 200,qaoa_on = 0):
    #每组数据 最后用map把数据找出来
    ...
    print("muuuuuu",mu)
        ...
        #矩阵
    sigma = -np.array(nx.adjacency_matrix(graph).todense())
        
    print("sigmaaaaa:",sigma)

    loss_function = Loss_function(graph)
        
    gas1 = GAS(len(list(graph.nodes)),loss_function,mu,sigma)
    gas2 = GAS(len(list(graph.nodes)),loss_function,mu,sigma)
    #gas1.show()
    
    maxiter = random.randint(50,60)
    
    opt_choice,_,total_R2,iter_time_depth2,y2= gas2.solver(shots,y_init,max_iter = maxiter)
    opt_choice,_,total_R1,iter_time_depth1,y1= gas1.solver(shots,y,max_iter = maxiter)

    print("correct_answer:",MC.brute_force_max_cut(edges,len(nodes)))
    #这里画图 value1 = none
    #怎么改参数
...
class GAS:
    ...
    def coefficients_trans(self,y):
        ...
        mu_max = np.max(np.abs(self.loss_mu))
        sigma_max = np.max(np.abs(self.loss_sigma))
        constant_max = np.max(np.abs(y))
        self.max = np.max([mu_max, sigma_max,constant_max])
        
        self.loss_mu_int = self.loss_mu
        self.loss_sigma_int = self.loss_sigma
        self.constant_int = y
    ...
    def solver(self, shots,y_1,lam=1.1, max_iter=60):

        self.coefficients_trans(y_1)
        print("self_constant",self.constant_int)
        k = 1
        i = 1
...
def MAx_cut(graph,p):
    ...
    theta_tmp = [1/p,1/p,1/p,1/p]
    print(theta_tmp)
    Max_reward,max_iter,depth= MC.Max_cut(theta_tmp,graph)
    print("Max_reward:{}".format(Max_reward))
    print("p:{},max_iter:{}".format(p,max_iter))

    LA = 20
    qa_iter_depth = max_iter * depth
    matrix_generator(graph,LA,Max_reward,1,p,qaoa_on = qa_iter_depth)
...
#ten_bit_version(data)

num_Qubits = 6
num_edge = 12
for i in range(10):
    graph = nx.DiGraph()
    edges = MC.generate_edges(num_Qubits,num_edge)    
    graph.add_nodes_from(range(0,num_Qubits))
